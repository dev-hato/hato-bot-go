...
// TestConnectRedirectErrorHasNoToken リダイレクト応答の処理でエラーになっても、
...
func TestConnectRedirectErrorHasNoToken(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			location: func(*http.Request) string {
				// Location自体にはトークンを含めず、CheckRedirectより前のURL解析を失敗させる。
				return "/invalid%zz"
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// リダイレクトを拒否する修正も許容し、追従やエラー文言そのものは必須条件にしない。
			if strings.Contains(err.Error(), tokenSecret) {
				t.Errorf("リダイレクト応答の処理エラーにトークンが露出している: %v", err)
			}
		})
	}
}
